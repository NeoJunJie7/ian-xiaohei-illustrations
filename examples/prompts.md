...
```text
Use $ian-xiaohei-illustrations 把下面这篇文章生成 4 张 xiaopi 诙谐正文配图。
要求：16:9 横版、纯白背景、黑色手绘线稿（或细黑描边）、少量红橙蓝中文手写批注。
...
请重新发明一个新的低科技隐喻，让 xiaopi 参与核心动作。
```
...
```text
Use $ian-xiaohei-illustrations 这张图方向对，但 xiaopi 有点像装饰。
请保持核心意思不变，重生成一版：让 xiaopi 成为真正推动结构运转的人。
画面更怪一点，但仍然纯白、清爽、少字。
```
...
```text
Use $ian-xiaohei-illustrations 输出 5 个不同主题的 xiaopi 正文配图效果。
主题分��覆盖：信息过载、产品验证、内容复利、一人公司、信任建立。
每张单独生成，不要拼成一张。
```
